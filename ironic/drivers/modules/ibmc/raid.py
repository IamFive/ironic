...
class IbmcRAID(base.RAIDInterface):
    ...
    def _delete_raid_configuration(self, task):
        ...
        with ibmc_client.connect(**ibmc) as conn:
            # - keep all delete logic in python-ibmcclient
            # - keep delete logic synchronized. if async required, do it in
            #   python-ibmcclient, make workflow in ironic simple.
    ...
    def _create_raid_configuration(self, task, logical_disks):
        ...
        with ibmc_client.connect(**ibmc) as conn:
            # - keep all delete logic in python-ibmcclient
            # - Create configuration logic should be synchronized. if async
            #   required, do it in python-ibmcclient.
    ...
    def create_configuration(self, task, create_root_volume=True,
        ...
        try:
            ...
            LOG.info(_("Succeed to create raid configuration on node %s."),
                     task.node.uuid)
        except exception.IBMCError as e:
            # - should we re-raise exception (NodeCleaningFailure)?
            # - is this action correct for both clean and deploy step?
            raise e
    ...
    def delete_configuration(self, task):
        ...
        try:
            ...
            # - should we re-raise exception (NodeCleaningFailure)?
            # - is this action correct for both clean and deploy step?
            # deploy_utils.set_failed_state(task, str(e), collect_logs=False)
            raise e
